...
if __name__ == '__main__':
    with open('16 nov 14_45 shift.json') as f:
        employees = json.loads(f.read())

    with open('16 nov 14_45 dm.json') as f:
        distance_matrix = json.loads(f.read())

        cab_types = ['INDICA', 'SUMO', 'TRAVELLER']
    price = [15, 15, 20]
    with open('16 nov 14_45 h2o.json') as f:
    ...
    with open('result 16 nov 14_45.json') as f:
    ...
    for i in range(3):
        if their_occupancy[i]!=0 and their_cab_count[i]!=0 :
    ...
    with open('our 16 nov 14_45.js', 'w') as f:
        f.write(json.dumps(our_stats))
    with open('their 16 nov 14_45.js', 'w') as f:
